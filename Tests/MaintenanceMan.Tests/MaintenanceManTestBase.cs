...
using MaintenanceMan.Migrations.SeedData;
...
namespace MaintenanceMan.Tests
{
    public abstract class MaintenanceManTestBase : AbpIntegratedTestBase<MaintenanceManTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MaintenanceManTestBase()
        {
        ...
        protected void UsingDbContext(Action<MaintenanceManDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<MaintenanceManDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MaintenanceManDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MaintenanceManDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MaintenanceManDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MaintenanceManDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<MaintenanceManDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MaintenanceManDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MaintenanceManDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MaintenanceManDbContext, Task<T>> func)
        {
            T result;
